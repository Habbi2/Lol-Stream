      ...
      <AnimatePresence>
        {highlights.map((e, i) => (
          <motion.div
            ...
            <div className="text-sm font-semibold text-white">{e?.payload?.title || e.title || 'Highlight'}</div>
            <div className="text-xs text-zinc-300">{e?.payload?.message || e.message || 'Nice play!'}</div>
          </motion.div>
        ))}
      </AnimatePresence>
    </div>
